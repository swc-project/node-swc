...
class Compiler extends native.Compiler {
  ...
  parseFileSync(src, options) {
    options = options || {};
    options.syntax = options.syntax || "ecmascript";
    return JSON.parse(super.parseFileSync(src, options));
  }
  ...
  print(m, options) {
    ...
    return new Promise((resolve, reject) => {
      super.print(JSON.stringify(m), options, (err, value) => {
        if (!!err) return reject(err);
        resolve(JSON.parse(value));
      });
    });
  ...
  printSync(m, options) {
    ...
    return JSON.parse(super.printSync(JSON.stringify(m), options));
  }
  ...
  transformSync(src, options) {
    ...
    return JSON.parse(super.transformSync(src, options, plugin));
  }
  ...
  transformFileSync(path, options) {
    ...
    return JSON.parse(super.transformFileSync(path, options, plugin));
  }
}
